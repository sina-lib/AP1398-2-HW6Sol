#ifndef __REAL_CUSTOMER_HEADER__
#define __REAL_CUSTOMER_HEADER__
#include "customer.h"
#include <string>

class RealCustomer : public Customer
{
private:
	long int nationalID;
	
public:
	
	RealCustomer(const std::string& name);
	RealCustomer(const std::string& name, long int nid);
	~RealCustomer();

	//virtual void importNewGoods( std::shared_ptr<BasicGoods> goods ) override;
	//virtual double getTotalAsset(void) const override;

	virtual bool operator==(const Customer& in) const override;
	virtual bool operator< (const Customer& in) const override;
	virtual bool operator< (const RealCustomer& in) const;
};

#endif // __REAL_CUSTOMER_HEADER__
